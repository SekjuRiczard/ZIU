...
export function TaskCardHiFi({ task, onClick }: TaskCardHiFiProps) {
  ...
  return (
    <motion.div
      ...
      className="bg-[var(--color-surface-card)] rounded-[var(--radius-lg)] shadow-[var(--shadow-md)] hover:shadow-[var(--shadow-lg)] cursor-pointer overflow-hidden transition-shadow duration-200"
      onClick={onClick}
    >
      ...
      <div className="p-[24px]">
        ...
        <div className="flex items-start justify-between mb-[12px]">
          <h3 className={`flex-1 ${task.status === 'completed' ? 'line-through text-[var(--color-text-secondary)]' : 'text-[var(--color-text-primary)]'}`}>
            {task.title}
          </h3>
          <span className={`px-[12px] py-[4px] rounded-[6px] text-[12px] font-medium ml-[12px] ${priorityColors[task.priority]}`}>
        ...
        <div className="flex items-center justify-between pt-[16px] border-t border-[var(--color-border)]">
          ...
          <div className={`flex items-center gap-[6px] text-[14px] ${
            task.status === 'completed' ? 'text-[var(--color-semantic-success)]' :
            task.status === 'overdue' ? 'text-[var(--color-semantic-error)]' :
